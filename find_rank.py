...
for rank in ranks:
    ...
    factorizer = UMF(X_train_raw, rank=rank, eta="lsearch", regularization=0, epsilon=1e-3, max_run=5000, verbose=True,
                     bias=False)
    factorizer.fit()
    ...
    pickle.dump(factorizer, open(os.path.join("trained_models","adaptive",filename), "wb"))

    print(f"Building estimator with rank wrt. bias {rank}")
    factorizer_bias = UMF(X_train_raw, rank=rank, eta="lsearch", regularization=0, epsilon=1e-3, max_run=5000, verbose=True,
                     bias=True)
    factorizer_bias.fit()
